...
const AssistantModal = ({ blogId, closeModal }) => {
  const validRole = ["Admin", "Author", "Supporter"];
  ...
  useEffect(() => {
    if (blogId > 0) {
      axios.get(`/api/Admin/blog/${blogId}`).then((response) => {
        if (response.data.assistantUserId > 0) {
          getUserInfo(response.data.assistantUserId);
        }
      });
    }
  }, [blogId]);
  ...
  const submitAssign = () => {
    if (userInfo && userInfo.id > 0) {
      axios
        .get(`/api/Admin/blog/changeAssistnt/${blogId}/${userInfo.id}`)
        .then((response) => {
      ...
      toast.error("Please choose Assistant from Assistant list.");
    }
  };
  ...
  return (
    <>
      <div className="modal-header">
        <h2>Assistant {userInfo.name ? " - " + userInfo.name : ""}</h2>
